...
public class MockSynth implements Synthesizer {
  ...
  public MockSynth() throws MidiUnavailableException {
    sb = new StringBuilder();
  }


  /**
   * Obtains information about the device, including its Java class and <code>Strings</code>
   ...
   *
   * @throws MidiUnavailableException thrown if the device cannot be opened due to resource
   *                                  restrictions.
   * @throws SecurityException        thrown if the device cannot be opened due to security
   *                                  restrictions.
   * @see #close
   * @see #isOpen
   */
  ...
  /**
   * Closes the device, indicating that the device should now release any system resources it is
   * using.
   *
   * <p>All <code>Receiver</code> and <code>Transmitter</code> instances open from this device are
   * closed. This includes instances retrieved via <code>MidiSystem</code>.
   *
   * @see #open
   * @see #isOpen
   */
  ...
  /**
   * Obtains the current time-stamp of the device, in microseconds. If a device supports
   * time-stamps, it should start counting at 0 when the device is opened and continue incrementing
   * its time-stamp in microseconds until the device is closed. If it does not support time-stamps,
   * it should always return -1.
   *
   * @return the current time-stamp of the device in microseconds, or -1 if time-stamping is not
  ...
  /**
   * Obtains the maximum number of MIDI IN connections available on this MIDI device for receiving
   * MIDI data.
   *
   * @return maximum number of MIDI IN connections, or -1 if an unlimited number of connections is
   * available.
   */
  @Override
  ...
  /**
   * Obtains the maximum number of MIDI OUT connections available on this MIDI device for
   * transmitting MIDI data.
   *
   * @return maximum number of MIDI OUT connections, or -1 if an unlimited number of connections is
   * available.
   */
  @Override
  ...
  /**
   * Obtains a MIDI IN receiver through which the MIDI device may receive MIDI data.  The returned
   * receiver must be closed when the application has finished using it.
   *
   * <p>Usually the returned receiver implements the {@code MidiDeviceReceiver} interface.
   *
   * <p>Obtaining a <code>Receiver</code> with this method does not open the device. To be able to
   * use the device, it has to be opened explicitly by calling {@link #open}. Also, closing the
   * <code>Receiver</code> does not close the device. It has to be closed explicitly by calling
   * {@link #close}.
   *
   * @return a receiver for the device.
   * @throws MidiUnavailableException thrown if a receiver is not available due to resource
   *                                  restrictions
   * @see Receiver#close()
   */
  @Override
  ...
  /**
   * Returns all currently active, non-closed receivers connected with this MidiDevice. A receiver
   * can be removed from the device by closing it.
   *
   * <p>Usually the returned receivers implement the {@code MidiDeviceReceiver} interface.
   *
  ...
  /**
   * Obtains a MIDI OUT connection from which the MIDI device will transmit MIDI data  The returned
   * transmitter must be closed when the application has finished using it.
   *
   * <p>Usually the returned transmitter implements the {@code MidiDeviceTransmitter} interface.
   *
   * <p>Obtaining a <code>Transmitter</code> with this method does not open the device. To be able
   * to use the device, it has to be opened explicitly by calling {@link #open}. Also, closing the
   * <code>Transmitter</code> does not close the device. It has to be closed explicitly by calling
   * {@link #close}.
   *
   * @return a MIDI OUT transmitter for the device.
   * @throws MidiUnavailableException thrown if a transmitter is not available due to resource
   *                                  restrictions
  ...
  /**
   * Returns all currently active, non-closed transmitters connected with this MidiDevice. A
   * transmitter can be removed from the device by closing it.
   *
   * <p>Usually the returned transmitters implement the {@code MidiDeviceTransmitter} interface.
   *
  ...
  /**
   * Obtains the processing latency incurred by this synthesizer, expressed in microseconds.  This
  ...
  /**
   * Informs the caller whether this synthesizer is capable of loading instruments from the
   * specified soundbank. If the soundbank is unsupported, any attempts to load instruments from it
   * will result in an <code>IllegalArgumentException</code>.
   *
   ...
   * @param to   the <code>Instrument</code> object to be used in place of the old instrument, it
   *             should be loaded into the synthesizer
   * @return <code>true</code> if the instrument successfully remapped, <code>false</code> if
   * feature is not implemented by synthesizer
   * @throws IllegalArgumentException if instrument <code>from</code> or instrument <code>to</code>
   *                                  aren't supported by synthesizer or if instrument
   *                                  <code>to</code> is not loaded
   * @throws NullPointerException     if <code>from</code> or <code>to</code> parameters have null
   *                                  value
   * @see #loadInstrument
   * @see #loadInstruments
   * @see #loadAllInstruments
  ...
  /**
   * Obtains the default soundbank for the synthesizer, if one exists. (Some synthesizers provide a
   * default or built-in soundbank.) If a synthesizer doesn't have a default soundbank, instruments
   * must be loaded explicitly from an external soundbank.
   *
   ...
   * learn whether another instrument can be loaded, you can invoke <code>isSoundbankSupported()
   * </code>, and if the instrument's <code>Soundbank</code> is supported, you can try loading the
   * instrument.
   *
  ...
  /**
   * Loads onto the <code>Synthesizer</code> all instruments contained in the specified
   * <code>Soundbank</code>.
   *
   ...
   * <code>Soundbank</code>.  Each of the <code>Patch</code> objects indicates a bank and program
   * number; the <code>Instrument</code> that has the matching <code>Patch</code> is loaded into
   * that bank and program location.
   *
  ...
  /**
   * Unloads the instruments referenced by the specified patches, from the MIDI sound bank
   * specified.
   *
   * @param soundbank soundbank containing instruments to unload
