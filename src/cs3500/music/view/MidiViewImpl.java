...
public class MidiViewImpl implements IMusicView {
  ...
  private void playNote(Note n) {
    ...
    MidiMessage stop;
    try {
      start = new ShortMessage(ShortMessage.NOTE_ON, getChannel(n.getInstrument()), n.getMidiPitch(), n.getVolume());
      stop = new ShortMessage(ShortMessage.NOTE_OFF, getChannel(n.getInstrument()), n.getMidiPitch(), n.getVolume());
  ...
  /**
   * Finds a channel in this.synth that is playing the given instrument. If no such channel can be found, chooses the least recently "allocated" channel of this method, and sets its program to instrument.
   *
  ...
  public void update(int beat) {
    for(Note n : this.model.notesToPlay(beat)) {
      this.playNote(n);
    }
  }
}
